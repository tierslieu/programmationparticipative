template = new Map([
  [
    "Chantier",
    [
      {
        name: "Participer au chantier",
        description:
          "Pense à prendre des chaussures fermées, de quoi te protéger du soleil, et tu peux prendre une douche sur place",
        min: 1,
        max: 99,
        active: true,
        slug: "Participer",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Préparation du repas bénévole",
        description: "",
        min: 1,
        max: 99,
        active: true,
        slug: "Repas",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Pêche des tilapia et préparation",
        description: "Pêcher les poissons de la serre et les préparer pour les conserver au froid",
        min: 1,
        max: 5,
        active: false,
        slug: "Tilapia",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Echauffement énergétique du matin",
        description: "Mobilisation musculaire en douceur pour mieux se préparer aux ateliers",
        min: 1,
        max: 8,
        active: true,
        slug: "Echauffement",
        minuteEnAvance: 15,
        minuteFinEnAvance: 225,
        creditsNegatif: true,
      },
      {
        name: "Animer un échauffement",
        description: "Animateur de l'échauffement",
        min: 1,
        max: 2,
        active: true,
        slug: "Animer",
        minuteEnAvance: 15,
        minuteFinEnAvance: 225,
      },
    ],
  ],

  [
    "Réunion",
    [
      {
        name: "Participer à la réunion",
        description:
          "Venir en tant que participant. Aucune connaissance particulière requise, seulement de la bonne volonté",
        min: 1,
        max: 20,
        active: false,
        slug: "Participer",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Animer la réunion",
        description:
          "Permettre que la réunion se passe dans de bonnes conditions en facilitant la prise de parole de toustes",
        min: 1,
        max: 2,
        active: true,
        slug: "Animer",
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
    ],
  ],

  [
    "Soirée",
    [
      {
        name: "Aide au Bar",
        description: "Aider au service des boissons. Un bienveillant est présent pour encadrer l'équipe. Aucune compétence particullière n'est requise, seule la bonne volonté et la bonne humeur suffisent",
        min: 2,
        max: 3,
        active: true,
        slug: "Bar",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
      {
        name: "Aide à la cuisine",
        description: "Aider à la préparation du repas et au dressage des assiettes. Un bienveillant est présent pour encadrer l'équipe. Aucune compétence particullière n'est requise, seule la bonne volonté et la bonne humeur suffisent",
        min: 2,
        max: 4,
        active: true,
        slug: "Cuisine",
        moitie: true,
        minuteEnAvance: 150,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-restauration.jpg",
      },
      {
        name: "Aide à l'accueil",
        description: "Accueillir les gens qui viennent à l'événement, prendre les nouvelles adhésions et faire les recharges de cartes cashless",
        min: 2,
        max: 3,
        active: true,
        slug: "Accueil",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-accueil.jpg",
      },
      {
        name: "Volontaire polyvalent",
        description: "couteau suisse",
        min: 1,
        max: 3,
        active: true,
        slug: "Polyvalent",
        moitie: true,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-volant.jpg",
      },
      {
        name: "Volontaire à la technique",
        description: "Nécessite d'avoir participé à une formation sur le matériel technique (sono / vidéo)",
        min: 1,
        max: 2,
        active: true,
        slug: "Technique",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
      {
        name: "Référent orga",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-orga",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent snack",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-snack",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent bar",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-bar",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent accueil",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-accueil",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: 0,
        creditFixe: 0,
      },                  
    ],
  ],

  [
    "Atelier",
    [
      {
        name: "Participer à l'atelier'",
        description:
          "Les ateliers sont des moments de pratique en commun. Chacun vient avec ce qu'il sait faire pour le partager ou apprendre des autres.",
        min: 1,
        max: 15,
        active: true,
        slug: "Participer",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Animer l'atelier",
        description:
          "L'animateur de l'atelier fait en sorte que l'atelier se déroule dans de bonnes conditions : il prévoit le matériel, l'espace, permet à chacun.e d'être inclus.e'",
        min: 1,
        max: 2,
        active: true,
        slug: "Animer",
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
    ],
  ],

  
  [
    "Café",
    [
      {
        name: "Référent du café-bar de la plume",
        description: "Aider au service des boissons. Un bienveillant est présent pour encadrer l'équipe. Aucune compétence particullière n'est requise, seule la bonne volonté et la bonne humeur suffisent",
        min: 1,
        max: 2,
        active: true,
        slug: "Bar",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
      {
        name: "Référent de la cuisine",
        description: "Décide du repas et mobilise les personnes pour s'assurer que les courses sont faites et que le repas se met en place pour au maximum 12h45. Pas de stress sur la cuisine, de la bonne volonté et la bonne humeur suffisent",
        min: 1,
        max: 2,
        active: true,
        slug: "Cuisine",
        moitie: true,
        minuteEnAvance: 150,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-restauration.jpg",
      },
      {
        name: "Aide à l'accueil",
        description: "Accueillir les gens qui viennent à l'événement, prendre les nouvelles adhésions et faire les recharges de cartes cashless",
        min: 2,
        max: 3,
        active: true,
        slug: "Accueil",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-accueil.jpg",
      },
      {
        name: "Volontaire polyvalent",
        description: "couteau suisse",
        min: 1,
        max: 3,
        active: true,
        slug: "Polyvalent",
        moitie: true,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
        image: "https://programme.laraffinerie.re/public/oceco-volant.jpg",
      },
      {
        name: "Volontaire à la technique",
        description: "Nécessite d'avoir participé à une formation sur le matériel technique (sono / vidéo)",
        min: 1,
        max: 2,
        active: true,
        slug: "Technique",
        moitie: false,
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
      {
        name: "Référent orga",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-orga",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent snack",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-snack",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent bar",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-bar",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: -30,
        creditFixe: 0,
      },
      {
        name: "Référent accueil",
        description: "Nécessite d'avoir participé à une formation sur le role de bienveillant",
        min: 1,
        max: 1,
        active: true,
        slug: "B-accueil",
        moitie: false,
        minuteEnAvance: 90,
        minuteFinEnAvance: 0,
        creditFixe: 0,
      },                  
    ],
  ],

  [
    "Atelier",
    [
      {
        name: "Participer à l'atelier'",
        description:
          "Les ateliers sont des moments de pratique en commun. Chacun vient avec ce qu'il sait faire pour le partager ou apprendre des autres.",
        min: 1,
        max: 15,
        active: true,
        slug: "Participer",
        minuteEnAvance: 0,
        minuteFinEnAvance: 0,
      },
      {
        name: "Animer l'atelier",
        description:
          "L'animateur de l'atelier fait en sorte que l'atelier se déroule dans de bonnes conditions : il prévoit le matériel, l'espace, permet à chacun.e d'être inclus.e'",
        min: 1,
        max: 2,
        active: true,
        slug: "Animer",
        minuteEnAvance: 30,
        minuteFinEnAvance: 0,
      },
    ],
  ],
  
  [
    "Formation",
    [
      {
        name: "Participer à la formation",
        description: "Participer à la formation pour apprendre ou développer quelque chose",
        min: 1,
        max: 20,
        active: true,
        slug: "Participer",
        minuteEnAvance: 00,
        creditsNegatif: true,
        minuteFinEnAvance: 0,
      },
      {
        name: "Animer la formation",
        description: "Le formateur crée le contenu de sa formation et la transmet aux participants",
        min: 1,
        max: 2,
        active: true,
        slug: "Animer",
        minuteEnAvance: 60,
        minuteFinEnAvance: 0,
      },
    ],
  ],
]);
